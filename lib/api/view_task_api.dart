...
Future<Map> getTasksListOnSpecificDate() async {
  FirebaseUser user = await FirebaseAuth.instance.currentUser();
  final uid = user.uid;
  ...
  for (var document in eventsQuery.documents) {
    ...
    for (int i = eventsQuery.documents.indexOf(document);
        i < eventsQuery.documents.length;
        i++) {
      String taskType = eventsQuery.documents[i].data["taskType"].toString();
      String specificTask =
          eventsQuery.documents[i].data["specificTask"].toString();
