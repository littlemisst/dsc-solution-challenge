...
class _UserProfileSummaryState extends State<UserProfileSummary> {
  ...
  Widget build(BuildContext context) {
    ...
    final bmi = _currentProfile.weight /
        (_currentProfile.height * _currentProfile.height);
    var formatter = new DateFormat('MM-dd-yyyy');
    String formatBirthDate = formatter.format(_currentProfile.birthDate);
    var difference =
        DateTime.now().difference(_currentProfile.birthDate).inDays;
    ...
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () =>
            Navigator.popAndPushNamed(context, Strings.editProfilePageRoute),
      ),
      body: SingleChildScrollView(
        ...
        child: Column(children: <Widget>[
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[
            ...
            SizedBox(width: 5.0),
            Text(_currentProfile.height.toString() + ' m',
                style: TextStyle(fontSize: 18.0)),
            SizedBox(width: 20.0),
            TextWidget(text: 'Weight:'),
            SizedBox(width: 5.0),
            Text(_currentProfile.weight.toString() + ' kg',
                style: TextStyle(fontSize: 18.0)),
          ]),
          Divider(),
          Row(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[
